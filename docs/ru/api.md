...
- [Управление категориями](#управление-категориями)
  ...
  - [Уникальные коды](#уникальные-коды-продукта)
    ...
    - [Получение веса уникальных кодов](#получение-веса-уникальных-кодов)
...
- `ignoreProductOutOfWeightRange` (boolean): Контролировать диапазон массы нетто продукта на линии аппликатора.
- `productMinWeight` (int): Минимальная допустимая масса нетто продукта, граммы.
- `productMaxWeight` (int): Максимальная допустимая масса нетто продукта, граммы.
...
#### Получение веса уникальных кодов
`POST /api/v1/categories/{categoryId}/unique-codes/weight`  
Возвращает ID продукта, вес нетто и вес тары для заданного списка уникальных кодов маркировки. Если код не найден, возвращаются `null` значения параметров.
...
**Одновременно может быть активно несколько партий** (по одной на производственную линию). При этом `name` и `external_id` уникальны глобально среди всех партий, включая закрытые, и обязательны при открытии партии.

### Список партий
`GET /api/v1/product-batches`  
Возвращает производственные партии, новые первыми.
Параметры запроса: `active` (необязательно) — `true` возвращает только активные партии (старые первыми), `false` — только закрытые, без параметра возвращаются все партии.
...
Возвращает одну партию.

**Ошибки**: `404`, если партия не найдена.
...
### Отчет по партии в разрезе категорий
`GET /api/v1/product-batches/{id}/categories`  
Возвращает продукты партии, агрегированные по категориям, в порядке названия категории. Вес — в граммах. Категории без продуктов в партии в отчет не попадают; продукты с удаленной категорией группируются в одну строку с `null` в `category_name` и `category_external_id`.
...
### Получение активной партии (устарело)
`GET /api/v1/product-batches/active`  
Возвращает **последнюю открытую** активную партию. Возвращает `404`, если активных партий нет.

_Устарело_: используйте `GET /api/v1/product-batches?active=true`. Endpoint сохранён для панелей аппликатора, которые ещё не поддерживают выбор партии: при любой ошибке кроме `404` такая панель продолжает использовать последнюю известную партию, поэтому удалять его нельзя до обновления всех панелей.
...
Открывает новую производственную партию. Уже открытые партии не закрываются.
Параметры запроса: `name`, `externalId`. Оба значения обрезаются по краям, не должны быть пустыми и не должны использоваться другой партией.

**Ошибки** (все `400 Bad Request`): пустое `name`; пустое `externalId`; `name` уже используется; `externalId` уже используется.
...
Закрывает указанную партию: пересчитывает статистику, устанавливает `end` и формирует отчёты. Возвращает закрытую партию.

**Ошибки**: `404`, если партия не найдена; `400`, если партия уже закрыта.

### Закрытие последней активной партии (устарело)
`POST /api/v1/product-batches/close`  
Закрывает **последнюю открытую** активную партию и возвращает её. Возвращает `404`, если активных партий нет.

_Устарело_: используйте `POST /api/v1/product-batches/{id}/close`. Endpoint сохранён для версий админ-панели, которые разворачиваются независимо от сервера.

### Отчет по партии
`GET /api/v1/product-batches/{id}/report`  
Возвращает сформированный CSV-отчёт по партии.
Параметры запроса: `type` — `BATCH` для сводного отчёта по партии, `PRODUCT` для отчёта по продуктам.

**Ошибки**: `400`, если отчёты ещё не сформированы (партия не закрыта); `404`, если партия или файл отчёта не найдены.
...
Экспорт производственных данных из настроенных исходных таблиц во внешние базы данных (PostgreSQL, MySQL, MS SQL Server, Oracle) по расписанию. Набор доступных для экспорта исходных таблиц (или представлений) задаётся свойством конфигурации `export.tables`; каждая должна иметь столбец `id`, используемый как водяной знак. Задача экспорта читает строки с `id` больше водяного знака задачи (`last_exported_id`) и вставляет их в целевую таблицу, поэтому каждый запуск переносит только новые записи.

Все эндпоинты экспорта данных требуют роль `ADMIN`.

### Список доступных таблиц
`GET /api/v1/export-tables`  
Возвращает имена исходных таблиц, доступных для экспорта, как задано в `export.tables`.
...
Возвращает столбцы доступной для экспорта исходной таблицы с их типами данных, прочитанные из локальной базы данных через JDBC-метаданные. `table` должна быть одной из таблиц, возвращаемых `GET /api/v1/export-tables`. Используется для построения маппинга столбцов.
...
`category` — одно из значений `NUMERIC`, `TEXT`, `BOOLEAN`, `DATETIME`, `BINARY`, `OTHER` — обобщённая группа SQL-типа для базовой проверки совместимости типов при маппинге столбцов.

### Подключения для экспорта

#### Список подключений
`GET /api/v1/export-connections`  
Возвращает все настроенные подключения к целевым базам данных. Пароли никогда не возвращаются.
...
`db_type` — одно из значений `POSTGRESQL`, `MYSQL`, `MSSQL` (MS SQL Server, включая редакции Express), `ORACLE`. Для `ORACLE` поле `database_name` — это имя сервиса (`jdbc:oracle:thin:@//host:port/service`). `properties` — необязательная строка дополнительных JDBC-параметров, добавляемых к URL подключения.

Перед сохранением подключение проверяется на целевой базе данных: запрос завершается ошибкой `400`, если база недоступна или учетные данные неверны.
...
Тело такое же, как при создании. Пустой или отсутствующий `password` сохраняет ранее сохраненный пароль. Как и при создании, обновленное подключение проверяется на целевой базе данных перед сохранением.

#### Удалить подключение
`DELETE /api/v1/export-connections/{id}`  
Возвращает `400`, если подключение все еще используется задачами экспорта.

#### Проверить подключение
`POST /api/v1/export-connections/{id}/test`  
Открывает подключение к целевой базе данных и проверяет его.
...
#### Таблицы целевой базы данных
`GET /api/v1/export-connections/{id}/tables`  
Читает таблицы целевой базы данных вместе с их столбцами через JDBC-метаданные. Ограничено текущим каталогом и схемой подключения (например, `public` для PostgreSQL, `dbo` для MSSQL, схема пользователя для Oracle). Используется UI для выбора целевой таблицы и целевых столбцов при создании задачи экспорта.
...
Читает имена столбцов таблицы в целевой базе данных (имя таблицы может включать схему, например `dbo.products`). Полезно для построения маппинга столбцов в UI.
...
- `source_table`: одна из таблиц, возвращаемых `GET /api/v1/export-tables`. Должна иметь столбец `id`.
- `column_mappings`: каждый `source` должен быть столбцом исходной таблицы (проверяется по актуальной схеме); `target` — столбец в целевой таблице. Каждая исходная и каждая целевая колонка может использоваться не более чем в одном сопоставлении.
- `cron_expression`: cron-выражение Spring из 6 полей (`секунда минута час день месяц день_недели`). Срабатывает в часовом поясе сервера (`server.timezone`). Необязательное: `null` или пустая строка означает, что задача запускается только вручную и не выполняется по расписанию — её можно запустить через эндпоинт ручного запуска.
- `enabled`: необязательное, по умолчанию `true`. Отключенные задачи не планируются.

**Файловые цели** (у подключения `target_type` = `FILE`): `target_table` не используется, `target` каждого сопоставления — заголовок колонки, а вместо этого применяются следующие поля:

- `delimiter`: обязательное, одно из `COMMA`, `PIPE`, `TAB`.
- `file_mask`: необязательный префикс имени файла; имя файла — маска, за которой следует метка времени `yyMMddHHmmss`.
- `file_extension`: необязательное, одно из `TXT`, `CSV`, `PSV`, `TSV`. По умолчанию — расширение разделителя (`CSV`, `PSV`, `TSV` соответственно).
- `start_header`, `end_header`: необязательные маркеры, записываемые первой и последней ячейкой строки заголовков. Они обозначают начало и конец заголовка и не имеют собственного столбца данных, поэтому строки данных ими не дополняются. Не должны содержать разделитель или перевод строки.
- `header_in_each_row`: необязательное, по умолчанию `false`. Если `true`, строка заголовков повторяется перед каждой строкой данных.

Последние три поля применяются только для расширения `TXT` и игнорируются для расширений семейства CSV. Например, при `PIPE`, `TXT`, `start_header` = `H_S`, `end_header` = `H_E`, заголовках `H1`, `H2`, `H3` и включенном `header_in_each_row` экспортируемый файл выглядит так:
...
**Ответ**: созданная задача, включая `last_exported_id` (водяной знак экспорта, начинается с `0`).

#### Обновить задачу
`PUT /api/v1/export-tasks/{id}`  
Тело такое же, как при создании. Изменение исходной таблицы сбрасывает водяной знак экспорта в `0`.

#### Удалить задачу
`DELETE /api/v1/export-tasks/{id}`  
Удаляет задачу из расписания вместе с логами ее запусков.

#### Запустить задачу сейчас
`POST /api/v1/export-tasks/{id}/run`  
Запускает асинхронное выполнение задачи вне расписания. Если задача уже выполняется, запуск пропускается.

#### Логи запусков задачи
`GET /api/v1/export-tasks/{id}/logs`  
Возвращает последние 100 запусков задачи, начиная с самых новых.
